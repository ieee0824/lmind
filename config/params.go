...
func DefaultParams() *Params {
	return &Params{
		...
		Stagnation: StagnationParams{
			DecayFrontal:    0.33,
			DecayTemporal:   0.22,
			DecayHypothesis: 0.34,
			BoostCuriosity:  0.05,
			BoostGrounding:  0.22,
			NoveltyRecovery: 0.13,
		},
		Hypothesis: HypothesisParams{
			DedupJaccard:      0.77,
			SentimentPositive: 0.69,
			SentimentNegative: -0.45,
		},
		PredictionError: PredictionErrorParams{
			HighThreshold:   0.91,
			MediumThreshold: 0.47,
		},
		Attention: AttentionParams{
			SalienceThreshold: 0.16,
			Baseline:          0.28,
			GoalWeight:        0.36,
		...
		Critic: CriticParams{
			RepetitionJaccard:   0.48,
			StagnationRepRate:   0.54,
			StagnationDominance: 0.30,
		},
		...
		Bonnou: BonnouParams{
			HotWordRetention:   196,
			IntensityThreshold: 0.50,
		},
